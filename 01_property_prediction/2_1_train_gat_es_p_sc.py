"""
Standard GAT Training Pipeline (The Winner Model)
=================================================
This script implements the Standard Graph Attention Network (GAT) architecture
which achieved the best performance (Real MAE ~0.56) on the Lipo dataset.

Configuration:
- Hidden Channels: 64 (Optimal capacity for this dataset size)
- Attention Heads: 4 (Balanced feature extraction)
- Regularization: None (No Dropout, pure GAT)
...
SCALER_PATH = os.path.join(SCRIPT_DIR, 'logp_gat_scaler.pkl')
MODEL_PATH = os.path.join(SCRIPT_DIR, 'best_gat_model.pth')
...
class GATModel(torch.nn.Module):
    """
    Standard GAT Model (The Winner).
    
    Architecture:
        - Hidden Channels: 64 (Optimized)
        - Attention Heads: 4
        - No Dropout (Pure Architecture)
    """
    ...
    def forward(self, x, edge_index, batch):
        # 1. Node Embedding Generation (ReLU only, No Dropout)
...
# =============================================================================
print("\n[Step 2] Initiating Training Process (Standard GAT)...")
